# Data log tidur yang akan disimpan
log_tidur = []

# Fungsi untuk menambahkan data tidur
...
# Fungsi untuk menampilkan seluruh data tidur
def tampilkan_log():
    if not log_tidur:
...
# Fungsi untuk menghitung rata-rata tidur
def rata_tidur():
    if not log_tidur:
...
# Fungsi untuk menampilkan tidur terlama dan tersingkat
def tidur_extrem():
    if not log_tidur:
...
# Fungsi untuk menampilkan 7 hari terakhir
...
# Menu interaktif untuk memilih fungsi
def menu():
    while True:
        print("\n--- Tracker Jam Tidur ---")
